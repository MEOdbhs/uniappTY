...
function getToolTipData(seriesData, opts, index, group, categories) {
  ...
  seriesData.forEach(function (item) {
    ...
    var formattedResult = option.formatter ? option.formatter(item, titleText, index, opts) : item.name + ': ' + item.data;
    // 如果 formatter 返回的是数组（多个 textList 项），则展开；否则作为单个项处理
    if (Array.isArray(formattedResult)) {
      formattedResult.forEach(function (textItem, idx) {
        textList.push({
          text: textItem.text || textItem,
          color: textItem.color !== undefined ? textItem.color : (idx === 0 ? item.color : null),
          textColor: textItem.textColor,
          legendShape: idx === 0 ? (opts.extra.tooltip.legendShape == 'auto' ? item.legendShape : opts.extra.tooltip.legendShape) : null
        });
      });
    } else {
      textList.push({
...
function calLegendData(series, opts, config, chartData, context) {
  ...
  let shapeWidth = ((typeof opts.legend.iconWidth === 'number' ? opts.legend.iconWidth : 15)) * opts.pix;
...
function drawToolTipSplitLine(offsetX, opts, config, context) {
  ...
  if (toolTipOption.splitAreaWidth) {
    var aw = toolTipOption.splitAreaWidth * opts.pix;
    context.beginPath();
    context.setFillStyle(hexToRgb(toolTipOption.splitAreaColor || '#4d84ff', toolTipOption.splitAreaOpacity != null ? toolTipOption.splitAreaOpacity : 0.15));
    context.rect(offsetX - aw / 2, startY, aw, endY - startY);
    context.closePath();
    context.fill();
  }
  if (toolTipOption.splitStroke === false) {
    context.setLineDash([]);
...
function drawToolTip(textList, offset, opts, config, context, eachSpacing, xAxisPoints) {
  ...
  textList.forEach(function (item, index) {
    ...
    context.setFillStyle((item.textColor !== undefined && item.textColor !== null) ? item.textColor : toolTipOption.fontColor);
...
function drawLegend(series, opts, config, context, chartData) {
  ...
  let shapeWidth = ((typeof opts.legend.iconWidth === 'number' ? opts.legend.iconWidth : 15)) * opts.pix;
  let iconHeight = ((typeof opts.legend.iconHeight === 'number' ? opts.legend.iconHeight : 10)) * opts.pix;
  let shapeRight = 5 * opts.pix;
  ...
  legendList.forEach(function (itemList, listIndex) {
    ...
    for (let i = 0; i < itemList.length; i++) {
      ...
      switch (item.legendShape) {
        case 'line':
          context.moveTo(startX, startY + 0.5 * lineHeight - iconHeight * 0.2);
          context.fillRect(startX, startY + 0.5 * lineHeight - iconHeight * 0.2, shapeWidth, iconHeight * 0.4);
          break;
        case 'triangle':
          const halfH = iconHeight / 2;
          context.moveTo(startX + shapeWidth / 2, startY + 0.5 * lineHeight - halfH);
          context.lineTo(startX, startY + 0.5 * lineHeight + halfH);
          context.lineTo(startX + shapeWidth, startY + 0.5 * lineHeight + halfH);
          context.lineTo(startX + shapeWidth / 2, startY + 0.5 * lineHeight - halfH);
          break;
        case 'diamond':
          const halfH2 = iconHeight / 2;
          context.moveTo(startX + shapeWidth / 2, startY + 0.5 * lineHeight - halfH2);
          context.lineTo(startX, startY + 0.5 * lineHeight);
          context.lineTo(startX + shapeWidth / 2, startY + 0.5 * lineHeight + halfH2);
          context.lineTo(startX + shapeWidth, startY + 0.5 * lineHeight);
          context.lineTo(startX + shapeWidth / 2, startY + 0.5 * lineHeight - halfH2);
          break;
        case 'circle':
          context.moveTo(startX + shapeWidth / 2, startY + 0.5 * lineHeight);
          context.arc(startX + shapeWidth / 2, startY + 0.5 * lineHeight, iconHeight / 2, 0, 2 * Math.PI);
          break;
        case 'rect':
          context.moveTo(startX, startY + 0.5 * lineHeight - iconHeight / 2);
          context.fillRect(startX, startY + 0.5 * lineHeight - iconHeight / 2, shapeWidth, iconHeight);
          break;
        case 'square':
          const sq = Math.min(shapeWidth, iconHeight);
          const sqOffsetX = startX + (shapeWidth - sq) / 2;
          context.moveTo(sqOffsetX, startY + 0.5 * lineHeight - sq / 2);
          context.fillRect(sqOffsetX, startY + 0.5 * lineHeight - sq / 2, sq, sq);
          break;
        case 'none':
          break;
        default:
