<template>
    <cl-page>
        <cl-topbar title="施工记录" background-color="#4474FF" color="#fff" :border="false"></cl-topbar>
                    ...
                    <view v-else class="document-list-inner">
                        <view v-for="(item, index) in documentList" :key="item.id || index" class="document-item"
                            ...
                            <view class="item-header">
                                ...
                                <view :class="[
                                    'status-tag',
                                    getStatusClass(item.statusDesc),
                                ]">
                                    {{ item.statusDesc }}
                                </view>
                            </view>
                            <view class="item-footer">
                                <text class="footer-time">{{ item.createDateTime || "-" }}</text>
                            </view>
                        </view>
                    </view>
                </cl-scroller>
...
import { getConstructionRecordList } from "./api"
...
const filterGroupList = [
    { id: "checkTime" as const, label: "施工时间" },
    { id: "checkStatus" as const, label: "施工状态" },
]

const checkStatusOptions = [
    { label: "下班", value: "下班" },
    { label: "正常施工", value: "正常" },
    { label: "加班施工", value: "加班施工" },
    { label: "停工", value: "停工" },
    { label: "未知", value: "未知" },
]
...
const selectedFilterChips = computed(() => {
    ...
    if (draftCheckStartTime.value) {
        chips.push({ label: `施工时间：${draftCheckStartTime.value}`, groupId: "checkTime" })
    }
...
function openFilterPopup() {
    draftCheckStartTime.value = queryParams.value.statusState || ""
    draftCheckStatus.value = queryParams.value.statusState || ""
...
function confirmFilter() {
    queryParams.value.statusDate = draftCheckStartTime.value
    queryParams.value.statusState = draftCheckStatus.value === "" ? "" : draftCheckStatus.value
...
async function fetchList() {
    ...
    loading.value = true
    try {
        const res = await getConstructionRecordList(queryParams.value)
...
function handleView(item: any) {
    router.push({
        path: "/pages/constructionRecord/detail",
        query: {
            mineId: item.mineId,
...
function getStatusClass(statusName: string): string {
    const statusMap: Record<string, string> = {
        "下班": "status-pending",
        "正常施工": "status-completed",
        "加班施工": "status-rectifying",
        "停工": "status-timeout",
        "未知": "status-unknown",
    }
    return statusMap[statusName] || "status-unknown"
}
